...
========================================
ResNeXt50 with 6-channel input, per-channel z-score normalization,
...
def main():
    print("=" * 60)
    print("ResNeXt50 with Controls and Plate Normalization")
    print("=" * 60)
    ...
    for epoch in range(Config.EPOCHS):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'best_resnext50_controls.pth')
            print(f"  → Saved best model: {best_acc:.2f}%")
    
    # Save final model
    torch.save(model.state_dict(), 'final_resnext50_controls.pth')
    print("\nSaved final model: final_resnext50_controls.pth")
    ...
    model.load_state_dict(torch.load('best_resnext50_controls.pth', map_location=device))
    ...
    submission.to_csv('submission_resnext50_controls.csv', index=False)
    
    print(f"\n{'='*60}")
    print(f"Complete! Best Val Acc: {best_acc:.2f}%")
    print("Saved submission_resnext50_controls.csv")
