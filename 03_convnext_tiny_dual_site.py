...
class RxRxDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for site in range(1, 3):
            for ch in range(1, 7):
                path = f"{DATA_DIR}/{self.mode}/{row['experiment']}/Plate{row['plate']}/{row['well']}_s{site}_w{ch}.png"
...
def get_model():
    ...
    model.features[0][0] = nn.Conv2d(
        12, 64, kernel_size=4, stride=4, padding=0, bias=False
    )

    with torch.no_grad():
        model.features[0][0].weight[:, :3] = original_conv.weight.repeat(4, 1, 1, 1) / 4
        model.features[0][0].weight[:, 3:6] = (
            original_conv.weight.repeat(4, 1, 1, 1) / 4
        )
        model.features[0][0].weight[:, 6:9] = (
            original_conv.weight.repeat(4, 1, 1, 1) / 4
        )
        model.features[0][0].weight[:, 9:] = original_conv.weight.repeat(4, 1, 1, 1) / 4
